...
describe('functionality', ()=>{
    ...
    it('has 3 rows with object with 2 keys', ()=>{
        const calibratedValueTable=mount(
            <CalibratedValueTable
                calibrated={{hello:5, goodbye:6}}
            />
        )
        expect(calibratedValueTable.find(TableRow).length).toEqual(3)
        expect(calibratedValueTable.find(TableRow).at(1).find(TableCell).at(0).text()).toEqual('hello')
        expect(calibratedValueTable.find(TableRow).at(1).find(TableCell).at(1).text()).toEqual('5.0000')
        expect(calibratedValueTable.find(TableRow).at(2).find(TableCell).at(0).text()).toEqual('goodbye')
        expect(calibratedValueTable.find(TableRow).at(2).find(TableCell).at(1).text()).toEqual('6.0000')
