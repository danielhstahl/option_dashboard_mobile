...
describe('functionality', ()=>{
  ...
  it('switches tabs when tab is clicked', ()=>{
    ...
    app.find(Tabs).props().onChange(null, 1)
    return delay(50)
      .then(()=>{
        app.update()
  ...
  it('calls getTicker correctly', ()=>{
    ...
    return delay(30)
      .then(()=>{
        app.find(SelectTicker).props().onChange({target:{
          value:'hello'
        }})
      })
      .then(()=>{
      ...
      .then(()=>{
        return expect(store.getState().marketValues.maturities).toEqual([3, 4, 5])
      })
  })
    ...
    return delay(30)
      ...
      .then(()=>{
        return expect(
            ...
            log_strike:5, transformed_option:3
          }])
      })
      .then(()=>{
        const {hello, world}=store
          .getState()
          .calibratorValues
          .attributes
        return expect(
          {hello, world}
        ).toEqual({hello:5, world:6})
      })
  })
})
